...
export function WeeklyShoppingListPage() {
  ...
  const startDate = useMemo(() => {
    ...
    const queryDate = params.get('startDate');
    if (queryDate) {
      return queryDate;
    }
  ...
  const storageKey = useMemo(() => {
    ...
    return `weeklyShoppingChecked:${userId}:${startDate}`;
  }, [userId, startDate]);
  ...
  const loadShoppingList = useCallback(async () => {
    ...
    try {
      ...
      setError(null);
      const token = getAccessToken();
      const data = await api.getWeeklyShoppingList(token, userId, startDate);
  ...
  if (!groupedItems.length) {
    return (
      <div className="weekly-shopping-page">
        ...
        <div className="shopping-state">No ingredients found for this week</div>
      </div>
    );
  }
